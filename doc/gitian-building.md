...
*Setup instructions for a gitian build of ucacoin Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the ucacoin
...
to the ucacoin GitHub release page.
...
- [Building ucacoin Core](#building-ucacoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ucacoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ucacoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ucacoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ucacoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ucacoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ucacoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ucacoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@ucacoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ucacoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ucacoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ucacoin-osx-*-build.assert
```
