UNIX BUILD NOTES
====================
Some notes on how to build ucacoin Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile ucacoin Core and the dependencies,
...
This will build ucacoin-qt as well, if the dependencies are met.
...
memory available when compiling ucacoin Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~ucacoin/+archive/ucacoin).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:ucacoin/ucacoin
    sudo apt-get update
...
To build ucacoin Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build ucacoin-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a ucacoin-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip ucacoind" to strip the debug
...
Security
--------
To help make your ucacoin Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, ucacoin Core should be built with a non-executable stack
