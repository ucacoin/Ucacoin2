...
class ReorgStakeTest(UCACoinTestFramework):
    ...
    def check_money_supply(self, expected_bc, expected_zuca):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected UCA and zUCA supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_bc))
            for denom in node['zUCAsupply']:
                assert_equal(node['zUCAsupply'][denom], DecimalAmt(expected_zuca[denom]))
    ...
    def run_test(self):
        ...
        # Check UCA and zUCA supply at the beginning
        # ------------------------------------------
        # zUCA supply: 2 coins for each denomination
        expected_zuca_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # UCA supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zuca_supply)
        ...
        # Verify that UCA and zUCA supplies were properly updated after the spends and reorgs
        self.log.info("Check UCA and zUCA supply...")
        ...
        expected_zuca_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zuca_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zuca_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zuca_supply)
