...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/UCACoinParams"
    else
        PARAMS_DIR="$HOME/.ucacoin-params"
    fi
fi
...
Failed to fetch the UCACoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
UCACoin - fetch-params.sh

This script will fetch the UCACoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common UCACoin zkSNARK parameters. Note that it is
