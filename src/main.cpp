...
#if defined(NDEBUG)
#error "ucacoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uuca) are considered zero fee (for relaying and mining)
...
CAmount GetMasternodeCollateral()
{
	...
	if (chainActive.Height() < 350000) return MASTERNODE_COLLATERAL;
	else if (chainActive.Height() < 450000) return MASTERNODE_COLLATERAL2;
	else if (chainActive.Height() < 550000) return MASTERNODE_COLLATERAL3;
	else if (chainActive.Height() < 650000) return MASTERNODE_COLLATERAL4;
	else if (chainActive.Height() < 750000) return MASTERNODE_COLLATERAL5;
	else if (chainActive.Height() >= 750000) return MASTERNODE_COLLATERAL6;
...
{
	bool isInvalid = nHeight >= Params().GetConsensus().height_start_InvalidUTXOsCheck;
	return !isInvalid;
}
...
{
	util::ThreadRename("ucacoin-scriptch");
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
	...
	if (outs >= 3 && lastOut.scriptPubKey != tx.vout[outs - 2].scriptPubKey) {
		...
		// cold staking is active much after height_start_ZC_PublicSpends so GetMasternodePayment is always 3 UCA.
		// TODO: double check this if/when MN rewards change
		CAmount blockValue = GetBlockValue(nHeight);
		CAmount masternodePayment = GetMasternodePayment(nHeight, blockValue, 0);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
	...
	for (int version = 2; version <= CBlockHeader::CURRENT_VERSION; version++) {
		if (block.nVersion < version && CBlockIndex::IsSuperMajority(version, pindexPrev, Params().GetConsensus().nRejectBlockOutdatedMajority)) {
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
	...
	if (pindexPrev && nHeight > 1500) { // pindexPrev is only null on the first block which is a version 1 block.
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> ucaInputs;

		for (const CTxIn& stakeIn : stakeTxIn.vin) {
			ucaInputs.push_back(stakeIn);
		}
		const bool hasUCAInputs = !ucaInputs.empty();
		...
		for (const CTransaction& tx : block.vtx) {
			for (const CTxIn& in : tx.vin) {
				...
				if (hasUCAInputs) {
					...
					for (const CTxIn& ucaIn : ucaInputs)
						if (ucaIn.prevout == in.prevout)
			...
			while (!chainActive.Contains(prev)) {
				...
				for (const CTransaction& t : bl.vtx) {
					...
					for (const CTxIn& in : t.vin) {
						...
						if (hasUCAInputs) {
							for (const CTxIn& stakeIn : ucaInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// ucacoin: We use certain sporks during IBD, so check to see if they are
