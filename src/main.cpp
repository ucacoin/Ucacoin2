...
/** Fees smaller than this (in uucacoin) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateUCASupply(int nHeightStart)
{
	...
	uiInterface.ShowProgress(_("Recalculating UCA supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating UCA supply..."), percent);
		}
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (!isPoSActive && nHeight > 1) {
		nSubsidy = 25000000 * COIN;//premimed
	}
	else if (nHeight <= 350000 && isPoSActive) {
		nSubsidy = 475 * COIN;
	}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
	...
	else if (nHeight <= 248200 && isPoSActive) {
		ret = blockValue * 0.30;
	}
	else  {
		ret = blockValue * 0.70;  //80%;
	}
...
bool IsSkippingRange(int nHeight) {
	if ((nHeight > 449990 && nHeight < 450120) || (nHeight > 549990 && nHeight < 550120) || (nHeight > 649990 && nHeight < 650120)
		|| (nHeight > 474990 && nHeight < 475050) || (nHeight > 749990 && nHeight < 750120))
	{
		return true;
	}
	return false;
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> ucacoinInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                ucacoinInputs.push_back(stakeIn);
        }
        const bool hasUCAInputs = !ucacoinInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasUCAInputs) {
                    ...
                    for (const CTxIn& ucacoinIn : ucacoinInputs)
                        if(ucacoinIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasUCAInputs) {
                            for (const CTxIn& stakeIn : ucacoinInputs)
