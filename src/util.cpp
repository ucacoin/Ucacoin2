...
const char * const UCACoin_CONF_FILENAME = "ucacoin.conf";
const char * const UCACoin_PID_FILENAME = "ucacoin.pid";
const char * const UCACoin_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\UCACoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\UCACoin
// Mac: ~/Library/Application Support/UCACoin
// Unix: ~/.ucacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ucacoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\UCACoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\UCACoinParams
    // Mac: ~/Library/Application Support/UCACoinParams
    // Unix: ~/.ucacoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UCACoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "UCACoinParams";
#else
    // Unix
    return pathRet / ".ucacoin-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", UCACoin_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", UCACoin_MASTERNODE_CONF_FILENAME));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty ucacoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override ucacoin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", UCACoin_PID_FILENAME));
