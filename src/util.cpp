...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "ucacoin" is a composite category enabling all ucacoin-related debug output
            if (ptrCategory->count(std::string("ucacoin"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ucacoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ucacoind.pid"));
