...
void CMasternode::Check(bool forceCheck)
{
    ...
    if (!unitTest) {
        ...
        CTxOut vout = CTxOut((GetMNCollateral(protocolVersion) - 0.01) * COIN, dummyScript);
...
bool CMasternode::IsInputAssociatedWithPubkey() const
{
    ...
    if(GetTransaction(vin.prevout.hash, txVin, hash, true)) {
        for (CTxOut out : txVin.vout) {
            if (out.nValue == GetMNCollateral(protocolVersion) * COIN && out.scriptPubKey == payee) return true;
        }
    }
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 33210) return false;
    } else if (addr.GetPort() == 33210)
        return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 UCA tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 UCACoin tx -> 1 confirmation
