...
std::string HelpMessage(HelpMessageMode mode)
{
	...
	strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in UCA) allowed by GUI and RPC for the stake split threshold (default: %s)"),
...
std::string LicenseInfo()
{
	return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		...
		"\n" +
		FormatParagraph(strprintf(_("Copyright (C) 2015-%i The PIVX Coin Developers"), COPYRIGHT_YEAR)) + "\n" +
		"\n" +
		FormatParagraph(strprintf(_("Copyright (C) %i The ucacoin developers"), COPYRIGHT_YEAR)) + "\n" +
		"\n" +
...
bool AppInit2()
{
	...
	// Sanity check
	if (!InitSanityCheck())
		return UIError(_("Initialization sanity check failed. UCACoin is shutting down."));
	...
	if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
		return UIError(strprintf(_("Cannot obtain a lock on data directory %s. UCACoin is probably already running."), strDataDir));
	...
	if (fDisableWallet) {
		...
		if (nLoadWalletRet != DB_LOAD_OK) {
			if (nLoadWalletRet == DB_CORRUPT)
				...
				strErrors << _("Error loading wallet.dat: Wallet requires newer version of UCACoin") << "\n";
			else if (nLoadWalletRet == DB_NEED_REWRITE) {
				strErrors << _("Wallet needed to be rewritten: restart UCACoin to complete") << "\n";
		...
		if (fFirstRun) {
			if (!fLegacyWallet) {
				...
				pwalletMain->SetMaxVersion(FEATURE_PRE_UCACoin);
			}
