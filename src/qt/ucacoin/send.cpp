...
#include "qt/ucacoin/send.h"
#include "qt/ucacoin/forms/ui_send.h"
#include "qt/ucacoin/addnewcontactdialog.h"
#include "qt/ucacoin/qtutils.h"
#include "qt/ucacoin/sendchangeaddressdialog.h"
#include "qt/ucacoin/optionbutton.h"
#include "qt/ucacoin/sendconfirmdialog.h"
#include "qt/ucacoin/myaddressrow.h"
#include "qt/ucacoin/guitransactionsutils.h"
...
SendWidget::SendWidget(ucacoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::send),
	...
	btnContacts(new QPushButton())
{
	...
	/* Subtitle */
	ui->labelSubtitle1->setText(tr("You can transfer public coins (UCA)"));
	...
	ui->labelAmountSend->setText("0.00 UCA");
	...
	setCssProperty(coinIcon, "coin-icon-uca");
...
void SendWidget::refreshAmounts() 
{
	...
	if (walletModel) {
		...
		ui->labelAmountRemaining->setText(GUIUtil::formatBalance(totalAmount, nDisplayUnit));
	}
}
...
void SendWidget::onSendClicked() {
	...
	// will call relock
	if (!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Send_UCA, true)) {
...
void SendWidget::onCoinControlClicked() {
	if (walletModel->getBalance() > 0) {
		...
		inform(tr("You don't have any UCA to select."));
	}
}
...
void SendWidget::onContactMultiClicked() {
	if (focusedEntry) {
		...
		CBitcoinAddress ucaAdd = CBitcoinAddress(address.toStdString());
		...
		if (dialog->res) {
			...
			if (walletModel->updateAddressBookLabels(ucaAdd.Get(), dialog->getLabel().toStdString(),
				AddressBook::AddressBookPurpose::SEND)) {
