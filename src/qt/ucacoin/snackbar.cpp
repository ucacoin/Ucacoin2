...
#include "qt/ucacoin/forms/ui_snackbar.h"
#include "qt/ucacoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(UCACoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &UCACoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
