...
#include "qt/ucacoin/addresseswidget.h"
#include "qt/ucacoin/forms/ui_addresseswidget.h"
#include "qt/ucacoin/addresslabelrow.h"
#include "qt/ucacoin/addnewaddressdialog.h"
#include "qt/ucacoin/tooltipmenu.h"

#include "qt/ucacoin/addnewcontactdialog.h"
#include "qt/ucacoin/ucacoingui.h"
#include "guiutil.h"
...
AddressesWidget::AddressesWidget(ucacoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter ucacoin address"));
    setCssProperty(ui->labelAddress, "text-title");
    ui->lineEditAddress->setPlaceholderText("e.g. GMeCEsjP732YA4uvBKR...");
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress ucaAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(ucaAdd.Get(), label.toUtf8().constData(),
                ucaAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
