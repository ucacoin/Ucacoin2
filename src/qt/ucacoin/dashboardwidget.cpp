...
#include "qt/ucacoin/dashboardwidget.h"
#include "qt/ucacoin/forms/ui_dashboardwidget.h"
#include "qt/ucacoin/sendconfirmdialog.h"
#include "qt/ucacoin/txrow.h"
...
DashboardWidget::DashboardWidget(UCACoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareUcaCoin, "square-chart-uca");
    setCssProperty(ui->labelUcaCoin, "text-chart-uca");
    ...
    setCssProperty(ui->labelAmountUca, "text-stake-uca-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &UCACoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &UCACoinGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &UCACoinGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("UCA")
    updateDisplayUnit();
}
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(20, 16, 11);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair UCA
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            uca = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalUca += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesUca.append(uca);

        int max = std::max(uca, uca);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(198, 157, 16));
    ...
    set0->append(chartData->valuesUca);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalUca > 0) {
        setCssProperty(ui->labelAmountUca, "text-stake-uca");
    } else {
        setCssProperty(ui->labelAmountUca, "text-stake-uca-disable");
    }
    forceUpdateStyle({ui->labelAmountUca});
    ui->labelAmountUca->setText(GUIUtil::formatBalance(chartData->totalUca, nDisplayUnit));
