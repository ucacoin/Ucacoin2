...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
	...
	if (nBlockHeight == 450000 || nBlockHeight == 550000 || nBlockHeight == 650000 || nBlockHeight == 750000) {
		std::vector<std::pair<int, CMasternode> > vMasternodeRanks = mnodeman.GetMasternodeRanks(nBlockHeight);

		for (PAIRTYPE(int, CMasternode) & s : vMasternodeRanks) {
			mnodeman.Remove(s.second.vin);
		}
		return true;
	}
	...
	if (IsSkippingRange(nBlockHeight)) {
